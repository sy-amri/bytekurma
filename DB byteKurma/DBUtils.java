...
public class DBUtils {
    ...
    public static void signUp(ActionEvent event, String username, String password, String confirmPassword) throws IOException {
        ...
        try (Connection connection = DriverManager.getConnection(JDBC_URL, USERNAME, PASSWORD)) {
            ...
            int userId = getUserID(username); // Assuming getUserID throws SQLException
            statement.setInt(1, userId);

            statement.executeUpdate();

            connection.commit(); // Commit the transaction (optional)

            changeScene(event, "mainPage.fxml", username);
        } catch (SQLException e) {
